...
void strassen_inverse_recursive_strassen(matrix *A, matrix *result, int limite)
{
    ...
    // Z=d-ceb
    strassen(e, b, temp, limite);
    strassen(c, temp, temp2, limite);
    matrice_substract(d, temp2, Z);

    // t=Z⁻1
    strassen_inverse_recursive_naive(Z, t);

    strassen(b, t, temp, limite);
    strassen(e, temp, temp2, limite);

    copyMatrix(temp2, y);

    negativeMatrix(y);

    strassen(c, e, temp3, limite);

    strassen(t, temp3, z, limite);
    negativeMatrix(z);
